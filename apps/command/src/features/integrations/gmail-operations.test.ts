...
describe("Command App Integration Configuration & Scopes", () => {
  it("defines Google Workspace integration with non-restricted scopes", () => {
    ...
    expect(googleInt?.scopes).toContain("https://www.googleapis.com/auth/gmail.send")
    expect(googleInt?.scopes).toContain("https://www.googleapis.com/auth/gmail.metadata")
    expect(googleInt?.scopes).toContain("https://www.googleapis.com/auth/calendar.events.readonly")
    expect(googleInt?.scopes).toContain("https://www.googleapis.com/auth/drive.file")

    // Ensure NO restricted scopes are included
    expect(googleInt?.scopes).not.toContain("https://mail.google.com/")
    expect(googleInt?.scopes).not.toContain("https://www.googleapis.com/auth/gmail.readonly")
    expect(googleInt?.scopes).not.toContain("https://www.googleapis.com/auth/gmail.modify")
  })
