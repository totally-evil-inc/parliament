...
import { useGmailThreadActivity, useRegisterGmailWatch } from "../hooks/use-gmail-operations"
...
export function GmailActivityHeatmap() {
  ...
  if (isLoading) {
    return <div className="p-4 text-xs text-muted-foreground">Loading Gmail activity heatmap...</div>
  }

  if (error) {
    return <div className="p-4 text-xs text-destructive">Failed to load Gmail activity heatmap.</div>
  }
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium">Gmail Real-Time Thread Activity</CardTitle>
          </div>
          <Badge
            variant={subscription?.status === "active" ? "secondary" : "outline"}
            className={subscription?.status === "active" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : ""}
          >
            {subscription?.status === "active" ? "Pub/Sub Active" : "Polling Mode"}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        {activities.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            No thread metadata events captured yet. Real-time Pub/Sub watcher is listening.
          </p>
        ) : (
            ...
            {activities.map((act) => (
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-muted-foreground text-[11px]">{act.subject || "No subject"}</span>
                </div>
                <div className="flex items-center gap-2">
                  {act.isSilent ? (
        ...
        <div className="pt-2">
          ...
          >
            ...
            {registerWatchMutation.isPending ? "Syncing..." : "Re-sync Real-time Pub/Sub Watcher"}
          </Button>
        </div>
      </CardContent>
    </Card>
