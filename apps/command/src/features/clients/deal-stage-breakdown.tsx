...
const STAGE_CONFIG: Record<
  string,
  { label: string; badgeBg: string; barBg: string }
> = {
  lead: {
    label: "Lead / Incoming",
    badgeBg:
...
export function DealStageBreakdown({ stageData }: Props) {
  ...
  return (
    <Card className="flex flex-col justify-between border-border bg-card shadow-xs">
      <CardHeader className="border-border/50 border-b pb-2">
            ...
            <div key={item.stage} className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <Badge
                    variant="outline"
                    className={`px-2 py-0.5 font-semibold text-[10px] ${config.badgeBg}`}
                  >
                    {config.label}
                  </Badge>
