...
describe("Public Document Backend API (apps/auth)", () => {
  let orgId: string
  ...
  describe("Proposals Server Logic", () => {
    test("getPublicProposalMeta returns proposal metadata for valid token", async () => {
      const res = await getPublicProposalMeta(proposalValidToken)
      expect(res.status).toBe("ready")
      if (res.status === "ready") {
        expect(res.token).toBe(proposalValidToken)
        expect(res.title).toBe("Gate Test Proposal")
        expect(res.sellerName).toBe("Test Seller")
      }
    })
  ...
  describe("Invoices Server Logic", () => {
    test("getPublicInvoiceMeta returns invoice metadata for valid token", async () => {
      const res = await getPublicInvoiceMeta(invoiceValidToken)
      expect(res.status).toBe("ready")
      if (res.status === "ready") {
  ...
  describe("Hono API Integration Routes in apps/auth", () => {
