...
describe("Agent Tool Catalog & Schemas", () => {
  ...
  it("validates create_proposal with stringified blocks, nested data wrappers, and aliases", () => {
    ...
    expect((parsed.blocks?.[0] as any).title).toBe("Executive Summary")
    expect(parsed.blocks?.[1].type).toBe("metrics")
    expect((parsed.blocks?.[1] as any).items).toHaveLength(2)
    expect(parsed.blocks?.[2].type).toBe("timeline")
    expect((parsed.blocks?.[2] as any).items).toHaveLength(1)
    expect(parsed.blocks?.[3].type).toBe("team")
    expect((parsed.blocks?.[3] as any).items).toHaveLength(1)
