...
export function RichTextInlineHtml({
  content,
  fallback = "",
}: Omit<RichTextHtmlProps, "className">) {
  const richContent =
    typeof content === "object" && content !== null
      ? content
      : fallback
...
function renderNode(node: RichTextNode): React.ReactNode {
  const children = node.content?.map((child, index) => (
    // biome-ignore lint/suspicious/noArrayIndexKey: read-only text nodes have no persistent unique IDs
  ...
  if (node.type === "inlineMath" || node.type === "blockMath") {
    ...
    return (
      <span
        ...
        // biome-ignore lint/security/noDangerouslySetInnerHtml: KaTeX math output is trusted
...
function renderMarks(
  ...
  return (marks ?? []).reduce<React.ReactNode>((node, mark) => {
    ...
    if (mark.type === "link") {
      ...
      // biome-ignore lint/correctness/useJsxKeyInIterable: chaining wrapper reduce
