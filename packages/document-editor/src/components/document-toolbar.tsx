...
export function DocumentToolbar({
  ...
  const handleItemMouseEnter = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setHoveredPill({
      left: targetRect.left - dockRect.left,
      top: targetRect.top - dockRect.top,
            ...
            <Button
              ...
              className={cn(
                ...
                isSend
                  ...
                  : "liquid-glass-item text-foreground/80 hover:text-foreground"
              )}
            />
          }
        >
          <action.icon className="pointer-events-none h-5 w-5" />
  ...
  return (
    <TooltipProvider delay={0}>
      {/* SVG Liquid Refraction Filter Primitive (Exact WebP base64 displacement map) */}
      <svg
        className="pointer-events-none fixed -left-[9999px] h-0 w-0"
        aria-hidden="true"
      >
        <defs>
          <filter id="liquid-glass-refract" primitiveUnits="objectBoundingBox">
            <feImage
              result="map"
              width="100%"
              height="100%"
      ...
      <div className="fixed bottom-6 left-1/2 z-50 hidden -translate-x-1/2 items-center md:flex">
        ...
        >
          ...
          {hoveredPill && (
            <div
              className="liquid-glass-pill top-0 left-0"
              style={{
                transform: `translate3d(${hoveredPill.left}px, ${hoveredPill.top}px, 0)`,
                width: `${hoveredPill.width}px`,
                height: `${hoveredPill.height}px`,
              }}
            />
          )}
          ...
          {layoutActions.length > 0 && blockActions.length > 0 && (
            <div className="liquid-glass-divider" aria-hidden="true" />
          )}
      ...
      {/* Mobile Compact Liquid Glass Dock */}
      <MobileCompactDock
...
function MobileCompactDock({
  ...
  const compactActions = definition.toolbarActions.filter(
    (action) =>
      action.togglesSidebar || action.id === "export" || action.id === "send"
  )
      ...
      >
        {compactActions.map((action) => {
          ...
          return (
            ...
            >
              <action.icon className="pointer-events-none h-4 w-4" />
              <span className="sr-only">{action.label}</span>
            </Button>
          )
        })}
